...
class ProfileFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val preferenceForLogin = requireActivity().getSharedPreferences("isLogin", AppCompatActivity.MODE_PRIVATE)
        val string = preferenceForLogin.getString("userId", "default")


        viewModel = ViewModelProvider(this, MyProfileViewModelFactory(contextI)).get(MyProfileViewModel::class.java)
    ...
    private fun addClickListener() {
        binding.signOutBtn.setOnClickListener {
            val preferenceForLogout = requireContext().getSharedPreferences("isLogin", AppCompatActivity.MODE_PRIVATE)
...
//            builder.create().show()
    ...
    private fun openCamera() {
        ...
        startActivityForResult(intent, REQUEST_CODE_CAMERA)                  // Request codes are declared in home activity.
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if(data != null) {
            ...
            if (resultCode == Activity.RESULT_OK && requestCode == REQUEST_CODE_CAMERA) {
                ...
                val path = MediaStore.Images.Media.insertImage(contextI.contentResolver, bitmapImage, "image", "description")
