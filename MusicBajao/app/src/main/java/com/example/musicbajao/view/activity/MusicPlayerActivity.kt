...
import com.example.musicbajao.viewModel.home.MusicPlayerViewModelFactory
...
class MusicPlayerActivity : AppCompatActivity(), View.OnClickListener, ServiceConnection {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityMusicPlayerBinding.inflate(layoutInflater)
        setContentView(binding.root)
        audioList = intent.getSerializableExtra("audiolist") as ArrayList<AudioModel>
        songPosition = intent.getIntExtra("position", -1)
    ...
    override fun onClick(p0: View?) {
        when (p0) {
            binding.playBtn -> {
                if (MusicPlayerViewModel.songPlaying.value != false) {
                    ...
                    musicService!!.showNotification(R.drawable.ic_play_button_back_foreground)
                } else {
                    viewModel.startPlaying()
                    musicService!!.showNotification(R.drawable.ic_pause_foreground)
                }
            }
