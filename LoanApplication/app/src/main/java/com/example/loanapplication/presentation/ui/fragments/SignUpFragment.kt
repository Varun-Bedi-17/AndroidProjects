...
import com.example.loanapplication.databinding.FragmentSignUpBinding
import com.example.loanapplication.presentation.ui.activites.HomeActivity
import com.example.loanapplication.presentation.viewmodels.SignUpViewModel
...
class SignUpFragment : Fragment() {
    ...
    private fun observeViewModel() {
        viewModel.signUpResult.observe(viewLifecycleOwner, EventObserver { result ->
            when(result){
                is ResponseState.Error -> showErrorState(result.message.toString())
                is ResponseState.Loading -> showLoadingState(result.data?.message)
    ...
    private fun showLoadingState(message: String?) {
        if(message == "Request Permissions"){
            if (!PermissionUtils.arePermissionsGranted(requireActivity())) {
